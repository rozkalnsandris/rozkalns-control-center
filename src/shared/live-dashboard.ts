...
function allowedActionsForLiveDecision(
  ...
  decision: Pick<DecisionReadModel, "workflowState" | "ci" | "review" | "issueNumber" | "prNumber">,
): DecisionReadModel["allowedActions"] {
  const actions: DecisionReadModel["allowedActions"] = [];
  const gitHubWriteReady = decision.issueNumber !== null && decision.prNumber !== null && decision.workflowState === "MERGE_READY" && decision.ci === "PASS" && isReviewRequirementSatisfied(decision.review);
...
async function readDecision(
  ...
  return {
    ...
    allowedActions: allowedActionsForLiveDecision(policy, { workflowState: state, ci, review, issueNumber, prNumber: pull.number }),
  };
}

function projectStatus(decisions: readonly DecisionReadModel[], openPullRequests: number): ProjectReadModel["status"] {
  if (decisions.some((item) => item.workflowState === "NEEDS_ANDRIS" || item.workflowState === "CI_FAILED")) return "ATTENTION";
  if (openPullRequests > 0) return "WAITING";
  return "HEALTHY";
}

async function readProject(factory: LiveDashboardReadContextFactory, policy: ManagedProjectPolicy, observedAt: string): Promise<{ project: ProjectReadModel; decisions: DecisionReadModel[] }> {
  ...
  if (repository.repository.toLowerCase() !== policy.repository.toLowerCase()) throw new Error("Live dashboard provider returned a different repository than requested");
...
export async function readLiveDashboardSnapshot(factory: LiveDashboardReadContextFactory, observedAtInput: string): Promise<ControlDashboardData> {
