-- Rozkalns Control Phase 4 source-only provider-neutral notification delivery intent registry.
...
CREATE TABLE notification_delivery_intents (
  ...
  target_key TEXT NOT NULL
    CHECK (length(target_key) BETWEEN 1 AND 64),
  ...
  UNIQUE (transition_id, target_key)
);
