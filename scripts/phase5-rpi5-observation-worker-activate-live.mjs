...
function parseCandidate(raw) {
  ...
  if (candidate.schema_version !== 1 || candidate.contract !== CANDIDATE_CONTRACT || candidate.repository !== REPO || candidate.worker !== WORKER_NAME) {
    stop("CANDIDATE_IDENTITY_INVALID", "candidate schema/contract/repository/worker mismatch");
  }
  if (!SHA1.test(candidate.source_sha) || !RUN_ID.test(candidate.ci_run_id) || !RUN_ID.test(candidate.preflight_run_id) || !SHA256.test(candidate.source_config_sha256)) {
    stop("CANDIDATE_SOURCE_IDENTITY_INVALID", "candidate source identity is malformed");
  }
  if (!exactKeys(candidate.toolchain, ["node_version", "wrangler_version"]) || candidate.toolchain.node_version !== NODE_VERSION || candidate.toolchain.wrangler_version !== WRANGLER_VERSION) {
  ...
  if (!exactKeys(current, ["deployment_id", "version_id", "traffic_percent", "ingest_binding_state", "verification_key_prerequisite", "d1_prerequisite", "non_target_bindings_sha256"])) {
    stop("CANDIDATE_BASELINE_SHAPE_INVALID", "candidate baseline fields drifted");
  }
  if (!UUID.test(current.deployment_id) || !UUID.test(current.version_id) || current.traffic_percent !== 100 || !["ABSENT", "PRESENT_FALSE"].includes(current.ingest_binding_state) || !SHA256.test(current.non_target_bindings_sha256)) {
    stop("CANDIDATE_BASELINE_INVALID", "candidate baseline identity is invalid");
  }
  const key = current.verification_key_prerequisite;
  if (!exactKeys(key, ["binding", "type", "state", "key_id", "provision_run_id", "value_observed"]) || key.binding !== KEY_BINDING || key.type !== "secret_text" || key.state !== "PRESENT_PROTECTED" || !KEY_ID.test(key.key_id) || !RUN_ID.test(key.provision_run_id) || key.value_observed !== false) {
  ...
  if (!exactKeys(d1, ["binding", "database_id", "migration_state"]) || d1.binding !== "CONTROL_DB" || d1.database_id !== DB_ID || d1.migration_state !== "PRESENT_VALID_0010_THROUGH_0013") {
    stop("CANDIDATE_D1_PREREQUISITE_INVALID", "candidate D1 prerequisite is invalid");
  }
  const intended = candidate.intended_result;
  if (!exactKeys(intended, ["version_strategy", "ingest_binding", "verification_key_binding", "d1_binding", "non_target_bindings_sha256", "routes", "custom_domains", "triggers"]) || intended.version_strategy !== VERSION_STRATEGY) {
    stop("CANDIDATE_RESULT_INVALID", "candidate version strategy drifted");
  }
  if (!exactKeys(intended.ingest_binding, ["name", "type", "value"]) || intended.ingest_binding.name !== INGEST_BINDING || intended.ingest_binding.type !== "plain_text" || intended.ingest_binding.value !== "true") {
    stop("CANDIDATE_INGEST_DELTA_INVALID", "candidate ingest delta is not exact plain-text true");
  }
  if (!exactKeys(intended.verification_key_binding, ["name", "type", "state", "key_id", "value_observed"]) || intended.verification_key_binding.name !== KEY_BINDING || intended.verification_key_binding.type !== "secret_text" || intended.verification_key_binding.state !== "PRESENT_PROTECTED_UNCHANGED" || intended.verification_key_binding.key_id !== key.key_id || intended.verification_key_binding.value_observed !== false) {
    stop("CANDIDATE_KEY_RESULT_INVALID", "candidate verification-key result is not unchanged/protected");
  }
  if (!exactKeys(intended.d1_binding, ["name", "database_id", "state"]) || intended.d1_binding.name !== "CONTROL_DB" || intended.d1_binding.database_id !== DB_ID || intended.d1_binding.state !== "UNCHANGED") {
    stop("CANDIDATE_D1_RESULT_INVALID", "candidate D1 result is not unchanged");
  }
  if (intended.non_target_bindings_sha256 !== current.non_target_bindings_sha256 || intended.routes !== "UNCHANGED" || intended.custom_domains !== "UNCHANGED" || intended.triggers !== "UNCHANGED") {
  ...
  if (!exactKeys(candidate.one_shot, ["authorization_consumed_at", "cross_class_cascade", "automatic_retry_rollback_cleanup_or_alternate_mutation", "after_upload_mismatch", "post_deploy_verification"]) || candidate.one_shot.authorization_consumed_at !== "FIRST_WORKER_VERSION_UPLOAD" || candidate.one_shot.cross_class_cascade !== false || candidate.one_shot.automatic_retry_rollback_cleanup_or_alternate_mutation !== false || candidate.one_shot.after_upload_mismatch !== "STOP_NO_DEPLOY" || candidate.one_shot.post_deploy_verification !== "GET_ONLY_EXACT_ACTIVE_VERSION_TRAFFIC_BINDINGS_AND_ROUTE_STATE") {
...
function assertExecutionContext(a) {
  ...
  if (!valid) stop("EXECUTION_CONTEXT_INVALID", "Worker activation requires the exact default-branch first-attempt GitHub-hosted workflow");
}
...
async function assertSource(a, candidate) {
  ...
  if (pkg?.engines?.node !== NODE_VERSION || pkg?.devEngines?.runtime?.version !== NODE_VERSION || pkg?.devDependencies?.wrangler !== WRANGLER_VERSION) stop("TOOLCHAIN_PIN_INVALID", "Node/Wrangler pins changed");
  const sourceConfigRaw = await readFile("wrangler.jsonc", "utf8");
  if (sha256(sourceConfigRaw) !== candidate.source_config_sha256) stop("SOURCE_CONFIG_SHA256_MISMATCH", "wrangler source config digest differs from candidate");
  const cfg = JSON.parse(sourceConfigRaw);
  if (cfg?.name !== WORKER_NAME || cfg?.vars?.[INGEST_BINDING] !== undefined) stop("SOURCE_CONFIG_BASELINE_INVALID", "source config must target the reviewed Worker and keep ingest dormant");
  const d1 = Array.isArray(cfg?.d1_databases) ? cfg.d1_databases : [];
  if (d1.length !== 1 || d1[0]?.binding !== "CONTROL_DB" || d1[0]?.database_id !== DB_ID) stop("CONTROL_DB_SOURCE_INVALID", "CONTROL_DB source binding changed");
...
async function assertGitHubEvidence(a, candidate) {
  ...
  if (ci?.name !== "CI" || ci?.path !== ".github/workflows/ci.yml" || ci?.head_branch !== "main" || ci?.head_sha !== a.sha || ci?.event !== "push" || ci?.status !== "completed" || ci?.conclusion !== "success") stop("CI_GATE_INVALID", "named CI is not successful exact-main push CI");
  const preflight = await gh(`/actions/runs/${a.preflightRun}`);
  if (preflight?.name !== "Phase 5 RPi5 observation GET/SELECT-only preflight" || preflight?.path !== PREFLIGHT_PATH || preflight?.head_branch !== "main" || preflight?.head_sha !== a.sha || preflight?.event !== "workflow_dispatch" || preflight?.status !== "completed" || preflight?.conclusion !== "success" || preflight?.run_attempt !== 1) stop("PREFLIGHT_GATE_INVALID", "named preflight is not successful first-attempt exact-main readonly evidence");
  const provision = await gh(`/actions/runs/${candidate.expected_current.verification_key_prerequisite.provision_run_id}`);
  if (provision?.name !== "Phase 5 RPi5 observation verification-key provision" || provision?.path !== PROVISION_PATH || provision?.head_branch !== "main" || provision?.head_sha !== a.sha || provision?.event !== "workflow_dispatch" || provision?.status !== "completed" || provision?.conclusion !== "success" || provision?.run_attempt !== 1) stop("VERIFICATION_KEY_PROVISION_GATE_INVALID", "named verification-key provision run is not successful exact-main first-attempt evidence");
  if (provision?.inputs && provision.inputs.key_id !== undefined && provision.inputs.key_id !== a.keyId) stop("VERIFICATION_KEY_PROVISION_KEY_ID_DRIFT", "provision run public key_id differs from authorization");
}
...
function assertBindingState(bindings, candidate, expectedIngest) {
  ...
  if (db.length !== 1 || db[0]?.type !== "d1" || (db[0]?.database_id ?? db[0]?.id ?? "") !== DB_ID) stop("CONTROL_DB_BINDING_DRIFT", "CONTROL_DB binding differs from reviewed prerequisite");
  const key = bindings.filter((binding) => binding?.name === KEY_BINDING);
  if (key.length !== 1 || key[0]?.type !== "secret_text" || Object.prototype.hasOwnProperty.call(key[0], "text")) stop("VERIFICATION_KEY_BINDING_DRIFT", "verification key is missing, exposed or not secret_text");
  const ingest = bindings.filter((binding) => binding?.name === INGEST_BINDING);
  if (expectedIngest === "ABSENT" && ingest.length !== 0) stop("INGEST_BASELINE_DRIFT", "ingest was expected absent");
  if (expectedIngest === "PRESENT_FALSE" && (ingest.length !== 1 || ingest[0]?.type !== "plain_text" || ingest[0]?.text !== "false")) stop("INGEST_BASELINE_DRIFT", "ingest was expected plain-text false");
  if (expectedIngest === "PRESENT_TRUE" && (ingest.length !== 1 || ingest[0]?.type !== "plain_text" || ingest[0]?.text !== "true")) stop("INGEST_CANDIDATE_INVALID", "candidate ingest binding is not exact plain-text true");
  if (nonTargetBindingsDigest(bindings) !== candidate.expected_current.non_target_bindings_sha256) stop("NON_TARGET_BINDING_DIGEST_DRIFT", "non-target binding inventory differs from candidate");
}
...
async function readWorkerState() {
  ...
  if (!UUID.test(current.id ?? "") || !UUID.test(versionId ?? "")) stop("WORKER_STATE_INVALID", "Worker deployment/version identifiers are invalid");
...
async function assertBaseline(a, candidate) {
  ...
  if (state.deployment !== a.deployment || state.version !== a.version) stop("WORKER_BASELINE_DRIFT", "active Worker differs from authorized deployment/version");
...
async function materializeCandidateConfig(sourceConfig) {
  const dir = await mkdtemp(join(tmpdir(), "phase5-worker-activate-"));
  const candidateConfig = structuredClone(sourceConfig);
  candidateConfig.vars = { ...(candidateConfig.vars ?? {}), [INGEST_BINDING]: "true" };
  const path = join(dir, "wrangler.phase5-worker-activate.jsonc");
  await writeFile(path, `${JSON.stringify(candidateConfig, null, 2)}\n`, { mode: 0o600 });
  return path;
}
...
function parseUploadedVersion(outputPath) {
  ...
  for (const line of raw.split(/\r?\n/)) {
    ...
    try {
      ...
      if (item?.type === "version-upload" && UUID.test(item?.version_id ?? "")) return item.version_id;
    } catch {
      // Ignore unrelated non-JSON output lines in the structured output file.
    }
  }
...
function requireRead(path) {
  const result = spawnSync(process.execPath, ["-e", "process.stdout.write(require('node:fs').readFileSync(process.argv[1],'utf8'))", path], {
    cwd: process.cwd(), encoding: "utf8", stdio: ["ignore", "pipe", "pipe"], env: cleanEnv(),
  });
...
function runWranglerWrite(args, outputPath) {
  ...
  if (result.error || result.status !== 0) stop("WRANGLER_WRITE_FAILED", `wrangler exited ${result.status ?? "unknown"}`);
}
...
async function uploadCandidate(a, candidateConfigPath) {
  ...
  runWranglerWrite([
    "versions", "upload", "--config", candidateConfigPath, "--strict",
  ...
  if (uploadedVersion === a.version) stop("UPLOADED_VERSION_EQUALS_BASELINE", "upload did not create a distinct version");
...
async function assertUploadedCandidate(versionId, candidate) {
  const version = await cf(`/workers/scripts/${WORKER_NAME}/versions/${versionId}`);
  if (version?.id !== versionId) stop("UPLOADED_VERSION_GET_MISMATCH", "GET did not return exact uploaded version");
  const bindings = Array.isArray(version?.resources?.bindings) ? version.resources.bindings : [];
  assertBindingState(bindings, candidate, "PRESENT_TRUE");
  console.log("UPLOADED_CANDIDATE_GET_VERIFY=PASS");
}

async function assertPredeployStillSafe(a) {
  const main = await gh("/branches/main");
  if (main?.commit?.sha !== a.sha) stop("MAIN_SHA_DRIFT_BEFORE_DEPLOY", "main moved after version upload");
  const state = await readWorkerState();
  if (state.deployment !== a.deployment || state.version !== a.version) stop("BASELINE_DRIFT_BEFORE_DEPLOY", "active production baseline changed after upload");
}
...
async function assertPostdeploy(a, candidate, versionId) {
  const main = await gh("/branches/main");
  if (main?.commit?.sha !== a.sha) stop("MAIN_SHA_DRIFT_POSTDEPLOY", "main moved during activation");
  const state = await readWorkerState();
  if (state.version !== versionId || state.version === a.version || state.deployment === a.deployment) stop("POSTDEPLOY_EXACT_VERSION_INVALID", "exact uploaded version is not the new active deployment");
  ...
  console.log("ROUTE_CUSTOM_DOMAIN_TRIGGER_QUEUE_MUTATION=NO");
...
async function main() {
  ...
  assertCredentials();
  if (sha256(a.candidateRaw) !== a.candidateSha256) stop("CANDIDATE_SHA256_MISMATCH", "exact manifest bytes do not match authorized candidate digest");
  const candidate = parseCandidate(a.candidateRaw);
  if (candidate.source_sha !== a.sha || candidate.ci_run_id !== a.ciRun || candidate.preflight_run_id !== a.preflightRun || candidate.expected_current.deployment_id !== a.deployment || candidate.expected_current.version_id !== a.version || candidate.expected_current.verification_key_prerequisite.key_id !== a.keyId) {
  ...
  await assertPredeployStillSafe(a);
...
main().catch((error) => {
  if (!process.exitCode) {
    console.error("STOP=UNEXPECTED_ERROR");
    if (mutationStarted) {
      console.error("POST_MUTATION_STATE=REVIEW_REQUIRED");
      console.error("AUTHORIZATION_CONSUMED=YES");
      console.error("NO_RETRY_ROLLBACK_CLEANUP_OR_ALTERNATE_MUTATION=YES");
  ...
  if (error?.name !== "Error" || !/^([A-Z0-9_]+)$/.test(error?.message ?? "")) console.error("UNEXPECTED_ERROR_REDACTED=YES");
});
