...
async function assertActivationSourceConfig() {
  ...
  const expectedConsumers = [
    { queue: MAIN_QUEUE_NAME, max_batch_size: 10, max_batch_timeout: 5, max_retries: 3, retry_delay: 30, max_concurrency: 1, dead_letter_queue: DLQ_NAME },
    { queue: DLQ_NAME, max_batch_size: 10, max_batch_timeout: 5, max_retries: 3, max_concurrency: 1 },
  ];
...
function exactQueue(queues, name, codePrefix) {
  ...
  if (typeof queue?.queue_id !== "string" || !OPAQUE_ID_PATTERN.test(queue.queue_id)) stop(`${codePrefix}_QUEUE_ID`, `${name} queue id was invalid`);
  return queue;
}
...
function assertConsumer(consumer, expected, codePrefix) {
  if (consumer?.type !== "worker" || consumer?.script_name !== WORKER_NAME) stop(`${codePrefix}_SCRIPT`, "Queue consumer is not bound to the reviewed Worker");
  ...
  if (expected.deadLetterQueue) {
    if (consumer?.dead_letter_queue !== expected.deadLetterQueue) stop(`${codePrefix}_DLQ`, "main Queue consumer dead-letter target changed");
...
async function assertActivatedQueues(apiToken, codePrefix) {
  ...
  if (mainConsumers.length !== 1 || dlqConsumers.length !== 1) stop(`${codePrefix}_CONSUMER_COUNT`, "each reviewed Queue must have exactly one consumer");
  assertConsumer(mainConsumers[0], { batchSize: 10, maxWaitTimeMs: 5000, maxRetries: 3, retryDelay: 30, maxConcurrency: 1, deadLetterQueue: DLQ_NAME }, `${codePrefix}_MAIN_CONSUMER`);
  assertConsumer(dlqConsumers[0], { batchSize: 10, maxWaitTimeMs: 5000, maxRetries: 3, maxConcurrency: 1 }, `${codePrefix}_DLQ_CONSUMER`);
  const producers = Array.isArray(main?.producers) ? main.producers : [];
  const workerProducers = producers.filter((producer) => producer?.type === "worker" && producer?.script === WORKER_NAME);
  if (workerProducers.length !== 1) stop(`${codePrefix}_PRODUCER`, "main Queue does not show the reviewed Worker as its producer");
...
function assertWebhookAccessAbsent(apps, codePrefix) {
  ...
  if (matches.length !== 0) stop(`${codePrefix}_WEBHOOK_APP_PRESENT`, "exact webhook Access application already exists; reconcile instead of rerunning activation");
}
...
async function readAccessJson(path, accessToken, codePrefix) {
  let response;
  try {
    response = await fetch(`https://${HOSTNAME}${path}`, { headers: { Accept: "application/json", "cf-access-token": accessToken }, signal: AbortSignal.timeout(15000) });
  } catch {
...
async function assertProtectedHealth(accessToken, codePrefix) {
  ...
  if (response.status !== 200 || body?.status !== "ok" || body?.service !== WORKER_NAME) stop(`${codePrefix}_HEALTH_CANARY`, "protected Control health canary did not match");
}
...
async function assertObservabilityReady(accessToken, codePrefix) {
  ...
  if (response.status !== 200 || !response.headers.get("cache-control")?.includes("no-store")) stop(`${codePrefix}_OBSERVABILITY_RESPONSE`, "webhook delivery observability is not ready/no-store");
  if (typeof body?.status !== "string" || typeof body?.totalDeliveries !== "number" || typeof body?.diagnosticsTruncated !== "boolean" || !Array.isArray(body?.diagnostics) || body.diagnostics.length > 50) {
...
async function assertPublicSignedPing(secret, codePrefix) {
  ...
  try {
    response = await fetch(`https://${WEBHOOK_ACCESS_DOMAIN}`, {
      method: "POST",
      headers: { Accept: "application/json", "Content-Type": "application/json", "X-GitHub-Delivery": "rozkalns-control-activation-ping", "X-GitHub-Event": "ping", "X-Hub-Signature-256": `sha256=${signature}` },
      body: payload,
      redirect: "manual",
      signal: AbortSignal.timeout(15000),
    });
  ...
  if (response.status !== 200 || body?.status !== "PING") stop(`${codePrefix}_PING_RESULT`, "signed webhook ping did not prove the public HMAC endpoint");
}

function versionIdSet(versions, codePrefix) {
  const ids = versions.map((version) => version?.id);
  if (ids.some((id) => typeof id !== "string" || id.length === 0) || new Set(ids).size !== ids.length) stop(`${codePrefix}_VERSION_SET_INVALID`, "Worker version inventory contained invalid or duplicate ids");
  return new Set(ids);
}

async function readFreshPrewrite(apiToken, audience, args, requireExpected) {
  const active = singleDeploymentVersion(await listDeployments(apiToken), "PREWRITE");
  if (requireExpected && (active.versionId !== args.currentVersion || active.deploymentId !== args.currentDeployment)) stop("PREWRITE_ACTIVE_STATE", "active Worker version/deployment moved from authorized baseline");
  ...
  const apps = await listAccessApps(apiToken);
  const parent = exactParentAccessApp(apps, audience, requireExpected ? args.accessAppId : "", "PREWRITE");
  ...
  if (requireExpected && deliveryCount !== Number(args.deliveryCount)) stop("PREWRITE_DELIVERY_COUNT", "D1 delivery baseline moved from authorized value");
...
async function cleanupSecretFile() {
  ...
  try { await rm(temporarySecretsDirectory, { recursive: true, force: true }); } finally { temporarySecretsDirectory = ""; }
}
...
function planOutput(args, prewrite) {
  ...
  console.log(`PARENT_ACCESS_APP_AUD=${prewrite.parent.aud}`);
  ...
  console.log(`OWNER_AUTHORIZATION=${AUTH_PREFIX}${args.sha} ci ${args.ci} version ${prewrite.active.versionId} deployment ${prewrite.active.deploymentId} domain ${prewrite.domain.id} access ${prewrite.parent.id} deliveries ${prewrite.deliveryCount} queues absent`);
...
async function plan(args) {
  ...
  const prewrite = await readFreshPrewrite(apiToken, audience, args, false);
...
async function apply(args) {
  ...
  const expectedAuthorization = `${AUTH_PREFIX}${args.sha} ci ${args.ci} version ${args.currentVersion} deployment ${args.currentDeployment} domain ${args.domainId} access ${args.accessAppId} deliveries ${args.deliveryCount} queues absent`;
  if (authorization !== expectedAuthorization) stop("OWNER_AUTHORIZATION_INVALID", "owner authorization does not match exact fresh activation baseline");
  ...
  await readFreshPrewrite(apiToken, audience, args, true);
  ...
  if (finalAudience !== audience) stop("FINAL_PREWRITE_ACCESS_AUDIENCE_CHANGED", "Access token audience changed during activation preflight");
  await readFreshPrewrite(apiToken, audience, args, true);
  const finalBeforeIds = versionIdSet(await listVersions(apiToken), "FINAL_PREWRITE");
  if (beforeIds.size !== finalBeforeIds.size || [...beforeIds].some((id) => !finalBeforeIds.has(id))) stop("FINAL_PREWRITE_VERSION_SET_CHANGED", "Worker version inventory changed during activation preflight");
  ...
  if (!OPAQUE_ID_PATTERN.test(mainQueue?.queue_id ?? "") || !OPAQUE_ID_PATTERN.test(dlq?.queue_id ?? "")) stop("QUEUE_CREATE_INVALID", "created Queue identities were invalid");

  const secretsFile = await prepareSecretFile(webhookSecret);
  try {
    run(wranglerPath(), ["deploy", "--name", WORKER_NAME, "--strict", "--experimental-provision=false", "--experimental-auto-create=false", "--install-skills=false", "--secrets-file", secretsFile], { inherit: true, env: sanitizedChildEnvironment(apiToken) });
  } finally {
  ...
  if (active.versionId !== newVersionId || active.deploymentId === args.currentDeployment) stop("POST_VERIFY_ACTIVE_DEPLOYMENT", "activation deploy did not become the unique 100% active Worker version");
  ...
  const postDeployAudience = accessAudience(accessToken, "POST_VERIFY");
  if (postDeployAudience !== audience) stop("POST_VERIFY_ACCESS_AUDIENCE_CHANGED", "Access application audience changed after Worker deploy");
  ...
  const finalAudienceAfter = accessAudience(accessToken, "FINAL");
  if (finalAudienceAfter !== audience) stop("FINAL_ACCESS_AUDIENCE_CHANGED", "parent Access application audience changed during activation");
