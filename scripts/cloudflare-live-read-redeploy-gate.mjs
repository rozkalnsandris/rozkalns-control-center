...
function assertLiveVersionBindings(detail, codePrefix) {
...
async function assertLiveSourceConfig() {
  ...
  if (config?.vars?.CONTROL_LIVE_READ_ENABLED !== "true") {
    stop("LIVE_READ_NOT_ENABLED", "live-read maintenance redeploy requires CONTROL_LIVE_READ_ENABLED=true");
  }
  if (
  ...
  for (const forbidden of ["route", "routes", "triggers", "custom_domain", "custom_domains"]) {
    if (Object.hasOwn(config, forbidden)) stop("SOURCE_ROUTING_PRESENT", `${forbidden} is forbidden in live-read redeploy source config`);
  }
...
async function assertExpectedActiveLive(apiToken, expectedVersionId, expectedDeploymentId, codePrefix) {
  ...
  assertLiveVersionBindings(await versionDetail(apiToken, expectedVersionId), codePrefix);
...
async function assertHealthCanary(codePrefix, accessToken) {
...
async function diagnosticReconcileCanary(codePrefix, accessToken) {
  ...
  if (!DIAGNOSTIC_ERROR_CODES.has(errorCode)) {
    stop(`${codePrefix}_RECONCILE_DIAGNOSTIC`, "diagnostic reconciliation did not return a bounded sanitized result");
  }
  return errorCode;
}
...
async function apply(args) {
  ...
  await assertExpectedActiveLive(apiToken, args.currentVersion, args.currentDeployment, "PREWRITE");
  await assertExactDomain(apiToken, args.domainId, "PREWRITE");
  await assertHealthCanary("PREWRITE", accessToken);
  ...
  if (beforeIds.size !== finalBeforeIds.size || [...beforeIds].some((id) => !finalBeforeIds.has(id))) {
    stop("FINAL_PREWRITE_VERSION_SET_CHANGED", "Worker version inventory changed during live-read redeploy preflight");
  }
  ...
  if (active.deploymentId === args.currentDeployment) {
    stop("POST_VERIFY_DEPLOYMENT_ID", "deployment id did not change after live-read redeploy");
  }

  assertLiveVersionBindings(await versionDetail(apiToken, newVersionId), "POST_VERIFY");
  ...
  console.log(`SANITIZED_DIAGNOSTIC_RESULT=${diagnosticResult}`);
  console.log("GITHUB_MUTATION=DISABLED");
  console.log("WEBHOOK_RUNTIME=DISABLED");
...
try {
  ...
  if (process.exitCode !== 1) {
    console.error("UNEXPECTED_FAILURE: sanitized live-read redeploy gate failure");
    process.exitCode = 1;
  }
}
