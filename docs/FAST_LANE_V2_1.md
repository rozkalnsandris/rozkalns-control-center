...
> Compatibility path: `AGENTS.md` already points to this v2.1 filename; the contents here are the authoritative v2.2 operating rules and supersede the v2.1 workflow semantics.

This repository keeps issue #1 and all GitHub/Cloudflare/RPi5 trust boundaries authoritative.
...
The Control Center target interaction is therefore:
...
For the current trusted Lenovo + Cloudflare Worker rollout pattern, one Composite Live authorization may explicitly allow both:
...
Examples of the presentation contract:

```bash
MERGE <repo>#<pr> HEAD=<exact-sha>
```

or, for a prepared local one-shot:

```bash
bash ~/Downloads/<exact-one-shot-script>.sh
```

Do not bury the required action inside explanatory prose or mix several optional approvals into one paragraph.
