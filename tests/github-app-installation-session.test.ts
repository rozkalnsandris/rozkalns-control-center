...
test("builds deterministic RS256 JWT claims and explicitly narrows installation-token scope", async () => {
  let signingInput = "";
...
test("fails closed when token repository evidence differs from requested scope", async () => {
  const provider = createGitHubAppInstallationSessionProvider({
    identity: { clientId: "control-client" },
    signer: signer(),
    fetchRequest: async () =>
...
test("fails closed when token permissions differ or expand beyond requested read scope", async () => {
...
test("rejects unusable expiry evidence from installation-token response", async () => {
...
test("maps installation-token endpoint auth and validation failures to fixed outcomes", async () => {
...
test("sanitizes signer and token transport failures", async () => {
...
test("rejects malformed app identity, observation time and empty signatures", async () => {
  assert.throws(
    () =>
      createGitHubAppInstallationSessionProvider({
  ...
  await assert.rejects(() => provider(scope(), "not-a-time"), (error) => errorCode(error) === "INVALID_TIME");

  const emptySignatureProvider = createGitHubAppInstallationSessionProvider({
    identity: { clientId: "control-client" },
    signer: { async signRs256() { return new Uint8Array(); } },
    fetchRequest: async () => jsonResponse(tokenPayload()),
  });
  await assert.rejects(
    () => emptySignatureProvider(scope(), observedAt),
    (error) => errorCode(error) === "SIGNING_FAILED",
  );
});

test("rejects malformed successful token responses without exposing response details", async () => {
  const malformedPayloads = [
    {},
    tokenPayload({ token: "" }),
    tokenPayload({ repositories: [] }),
  ...
  for (const payload of malformedPayloads) {
    const provider = createGitHubAppInstallationSessionProvider({
      identity: { clientId: "control-client" },
      signer: signer(),
      fetchRequest: async () => jsonResponse(payload),
    });
    await assert.rejects(
      () => provider(scope(), observedAt),
      (error) => error instanceof GitHubAppSessionError && error.message.startsWith("GitHub App installation token"),
    );
  }
...
test("authorized session attaches the opaque installation credential only inside fixed GitHub GET execution", async () => {
  ...
  assert.equal(requests[1].headers.get("accept"), GITHUB_REST_ACCEPT);
  assert.equal(requests[1].headers.get("x-github-api-version"), GITHUB_REST_API_VERSION);
  assert.equal(JSON.stringify(result).includes("test-only-opaque-installation-credential"), false);
});

test("authorized session rejects off-origin or out-of-scope reads before HTTP", async () => {
  let fetchCount = 0;
  const provider = createGitHubAppInstallationSessionProvider({
    identity: { clientId: "control-client" },
    signer: signer(),
  ...
  await assert.rejects(
    () =>
      session.execute({
        method: "GET",
        url: "https://example.invalid/repos/rozkalnsandris/hermes-tech/pulls/42",
  ...
  await assert.rejects(
    () =>
      session.execute({
        method: "GET",
        url: "https://api.github.com/repos/rozkalnsandris/hermes-deals/pulls/42",
        accept: GITHUB_REST_ACCEPT,
        apiVersion: GITHUB_REST_API_VERSION,
        redirect: "manual",
      }),
...
test("authorized read transport failures are sanitized", async () => {
  const provider = createGitHubAppInstallationSessionProvider({
    identity: { clientId: "control-client" },
    signer: signer(),
    fetchRequest: async (request) => {
  ...
  await assert.rejects(
    ...
    (error) =>
      error instanceof GitHubAppSessionError &&
      error.code === "READ_TRANSPORT_FAILED" &&
